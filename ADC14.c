...
// Takes the src voltage (3300 mV or 5000 mV)
// Returns the voltage in mV on the ADC input pin
unsigned int ADC_Voltage(int vsrc)
{
    unsigned int analogReading = ADC_In();
    
    return ADC_GetVoltageFromInput(analogReading, vsrc);
}

/* Takes the 16 bit input val from the ADC (vin)
    and the src voltage of either3300 mV or 5000 mV (vsrc)
   Returns the voltage in mV on the ADC input pin
*/
unsigned int ADC_GetVoltageFromInput(int vin, int vsrc)
{
    return (((vin*vsrc))/((1024*16)-1));
}
